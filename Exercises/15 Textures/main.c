...
static MOTION_TYPE lightMotionType = 0;
...
static float lights[] = {
	1, 0, 0, 0.8660, -2, 0, 0, 128, -1, 0, 0, 0.5,	// Luz Roja
...
static void initLightCubes()
{
	float l1 = -0.2, l2 = 0.2;
	float positions[] = {
		l1, l1, l2, l2, l1, l2, l1, l2, l2, l2, l1, l2, l2, l2, l2, l1, l2, l2, // Frente
...
static void displayFunc()
{
	...
	{
	case LEFT:
		if (cameraX - cameraSpeed > w1)
			cameraX -= cameraSpeed;
		break;
	case RIGHT:
		if (cameraX + cameraSpeed < w2)
			cameraX += cameraSpeed;
		break;
	case FRONT:
		if (cameraZ - cameraSpeed > d1)
			cameraZ -= cameraSpeed;
		break;
	case BACK:
		if (cameraZ + cameraSpeed < d2)
			cameraZ += cameraSpeed;
		break;
	case UP:
	case DOWN:
	...
	{
	case LEFT:
		if (greenLightX - greenLightSpeed > w1)
			greenLightX -= greenLightSpeed;
		break;
	case RIGHT:
		if (greenLightX + greenLightSpeed < w2)
			greenLightX += greenLightSpeed;
		break;
	case UP:
		if (greenLightY + greenLightSpeed < h2)
			greenLightY += greenLightSpeed;
		break;
	case DOWN:
		if (greenLightY - greenLightSpeed > h1)
			greenLightY -= greenLightSpeed;
		break;
	case FRONT:
		if (greenLightZ - greenLightSpeed > d1)
			greenLightZ -= greenLightSpeed;
		break;
	case BACK:
		if (greenLightZ + greenLightSpeed < d2)
			greenLightZ += greenLightSpeed;
		break;
	case IDLE:;
	}
...
static void keyPressedFunc(unsigned char key, int x, int y)
{
	...
	case 'a':
	case 'A':
		lightMotionType = LEFT;
		break;
	case 'd':
	case 'D':
		lightMotionType = RIGHT;
		break;
	case 'w':
	case 'W':
		lightMotionType = UP;
	...
	case 's':
	case 'S':
		lightMotionType = DOWN;
		break;
	case 'r':
	case 'R':
		lightMotionType = FRONT;
		break;
	case 'f':
	case 'F':
		lightMotionType = BACK;
		break;
	case 27:
		exit(0);
	}
